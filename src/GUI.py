...
class App:
    ...
    def _list_preview_ch_files(self) -> list[Path]:
        ...
        matches: list[Path] = []
        for pip_folder, _folder_number, _data_name in folder.build_pip_targets(parent_dir):
    ...
    def refresh_header_preview(self) -> None:
        ...
        try:
            ...
            header = ch.extract_header(data)
            decoded = ch.decode_signal_from_offset(data, int(header["header_size_bytes"]))
            decoded_count = len(decoded)
            first_time_ms = header.get("first_time_ms")
            last_time_ms = header.get("last_time_ms")
            point_count = ch.deduce_output_point_count(first_time_ms, last_time_ms, decoded_count)
